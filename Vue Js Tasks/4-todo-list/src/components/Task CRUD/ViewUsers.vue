  ...
  <div class="dashboard-wrapper">
    ...
    <div class="dashboard-user-list">
      ...
      <div v-for="(user, index) in userData" :key="user.id">
        ...
        <div class="user-data-container" v-if="index === editIndex">
          <input class="user-detail user-detail-input" ref="inputName" :value="user.name" />
          <input class="user-detail user-detail-input" ref="inputSurname" :value="user.surname" />
          <input class="user-detail user-detail-input" ref="inputAge" :value="user.age" />
          <input class="user-detail user-detail-input" ref="inputDob" :value="user.dob" />
          <input class="user-detail user-detail-input" ref="inputGender" :value="user.gender" />

          <div>
            <button @click="saveChanges(user.id)" class="edit-btn">Save</button>
            <button @click="cancelChanges()" class="edit-btn">Cancel</button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    editUser(userId) {
      this.editIndex = this.userData.findIndex((user) => user.id === userId);
    },

    saveChanges(){

        this.userData[this.editIndex].name = this.$refs.inputName[0].value
        this.userData[this.editIndex].surname = this.$refs.inputSurname[0].value
        this.userData[this.editIndex].age = this.$refs.inputAge[0].value
        this.userData[this.editIndex].dob = this.$refs.inputDob[0].value
        this.userData[this.editIndex].gender = this.$refs.inputGender[0].value
    ...
    deleteUser(userId) {
      const userIndex = this.userData.findIndex((user) => user.id === userId);
